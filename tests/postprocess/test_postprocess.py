...
def test_lorentzian_single_state_peak_at_energy():
    ...
    state = _state(1, energy_ev=10.0, osc=1.0)
...
def test_lorentzian_only_singlets_skips_triplets():
    ...
    singlet = _state(1, energy_ev=10.0, osc=1.0, multiplicity="Singlet")
    triplet = _state(2, energy_ev=12.0, osc=1.0, multiplicity="Triplet")
...
def test_lorentzian_all_filtered_returns_zeros():
    ...
    triplet = _state(1, energy_ev=10.0, osc=1.0, multiplicity="Triplet")
...
def test_lorentzian_none_oscillator_skipped(caplog):
    ...
    none_state = _state(1, energy_ev=10.0, osc=None)
    grid = np.linspace(5.0, 15.0, 100)
...
def test_lorentzian_intensity_positive():
    ...
    states = [_state(i, float(e), osc=0.5) for i, e in enumerate([8.0, 10.0, 12.0], start=1)]

...
@pytest.mark.parametrize(
    "parsed_qchem_h2o_sp_data,expected_wall_time,expected_cpu_time",
    [
...
def test_sp_time_values(
    ...
    """
    assert parsed_qchem_h2o_sp_data.timing is not None
    assert parsed_qchem_h2o_sp_data.timing.total_wall_time_seconds == pytest.approx(expected_wall_time, abs=TIME_TOL)
    assert parsed_qchem_h2o_sp_data.timing.total_cpu_time_seconds == pytest.approx(expected_cpu_time, abs=TIME_TOL)


@pytest.mark.regression
@pytest.mark.parametrize(
    "parsed_qchem_h2o_tddft_data,expected_wall_time,expected_cpu_time",
    [
    ...
    indirect=["parsed_qchem_h2o_tddft_data"],
    ids=["tddft-uks-5.4", "tddft-uks-6.2", "tddft-rks-6.2"],
)
def test_tddft_time_values(
    parsed_qchem_h2o_tddft_data: CalculationResult, expected_wall_time: float, expected_cpu_time: float
):
    """
    Regression test: verify wall and CPU time have correct values for TDDFT calculations.
    """
    assert parsed_qchem_h2o_tddft_data.timing is not None
    assert parsed_qchem_h2o_tddft_data.timing.total_wall_time_seconds == pytest.approx(expected_wall_time, abs=TIME_TOL)
    assert parsed_qchem_h2o_tddft_data.timing.total_cpu_time_seconds == pytest.approx(expected_cpu_time, abs=TIME_TOL)

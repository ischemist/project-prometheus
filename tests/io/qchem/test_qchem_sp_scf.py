...
Q-Chem output files. Values are hardcoded from ex-scf.md (Q-Chem 6.2).
...
# =============================================================================
# HARDCODED TEST DATA FROM ex-scf.md (Q-Chem 6.2 format)
# =============================================================================

# SCF Iterations - sample for regression testing
EXPECTED_ITERATIONS = [
...
def test_scf_results_has_correct_type(parsed_qchem_h2o_sp_data: CalculationResult):
    """Contract test: verify scf field is ScfResults instance."""
    assert parsed_qchem_h2o_sp_data.scf is not None
    assert isinstance(parsed_qchem_h2o_sp_data.scf, ScfResults)


@pytest.mark.contract
def test_scf_results_iterations_is_tuple(parsed_qchem_h2o_sp_data: CalculationResult):
    """Contract test: verify iterations field is a tuple of ScfIteration objects."""
    assert parsed_qchem_h2o_sp_data.scf is not None
    assert isinstance(parsed_qchem_h2o_sp_data.scf.iterations, tuple)
    assert len(parsed_qchem_h2o_sp_data.scf.iterations) > 0
    assert all(isinstance(it, ScfIteration) for it in parsed_qchem_h2o_sp_data.scf.iterations)


@pytest.mark.contract
def test_scf_results_required_fields_present(parsed_qchem_h2o_sp_data: CalculationResult):
    """Contract test: verify all required ScfResults fields are present with correct types."""
    scf = parsed_qchem_h2o_sp_data.scf
...
@pytest.mark.contract
def test_scf_iteration_required_fields(parsed_qchem_h2o_sp_data: CalculationResult):
...
@pytest.mark.contract
def test_scf_n_iterations_matches_length(parsed_qchem_h2o_sp_data: CalculationResult):
...
@pytest.mark.contract
def test_smd_results_has_correct_type(parsed_qchem_h2o_sp_data: CalculationResult):
    """Contract test: verify smd field is SmdResults instance when present."""
    assert parsed_qchem_h2o_sp_data.smd is not None
    assert isinstance(parsed_qchem_h2o_sp_data.smd, SmdResults)


@pytest.mark.contract
def test_smd_results_has_expected_fields(parsed_qchem_h2o_sp_data: CalculationResult):
    """Contract test: verify SmdResults has expected fields populated."""
    smd = parsed_qchem_h2o_sp_data.smd
...
@pytest.mark.contract
def test_final_energy_set_from_smd_total(parsed_qchem_h2o_sp_data: CalculationResult):
    """
    ...
    assert parsed_qchem_h2o_sp_data.final_energy is not None
    assert parsed_qchem_h2o_sp_data.smd is not None
    # They should be the same value (within tolerance)
    assert parsed_qchem_h2o_sp_data.final_energy == pytest.approx(parsed_qchem_h2o_sp_data.smd.g_tot_au, abs=ENERGY_TOL)
...
def test_scf_parsed_alongside_metadata(parsed_qchem_h2o_sp_data: CalculationResult):
    """
    ...
    """
    assert parsed_qchem_h2o_sp_data.metadata is not None
    assert parsed_qchem_h2o_sp_data.metadata.software_version is not None

    assert parsed_qchem_h2o_sp_data.scf is not None


@pytest.mark.integration
def test_scf_parsed_alongside_geometry(parsed_qchem_h2o_sp_data: CalculationResult):
    """
    ...
    assert len(parsed_qchem_h2o_sp_data.input_geometry) == 3  # H2O

    assert parsed_qchem_h2o_sp_data.scf is not None


@pytest.mark.integration
def test_scf_completion_flag_set(parsed_qchem_h2o_sp_data: CalculationResult):
    """
...
@pytest.mark.integration
def test_scf_final_energy_populated(parsed_qchem_h2o_sp_data: CalculationResult):
    """Integration test: verify final_energy is set after SCF parsing."""
    assert parsed_qchem_h2o_sp_data.final_energy is not None
    assert isinstance(parsed_qchem_h2o_sp_data.final_energy, float)


@pytest.mark.integration
def test_scf_and_smd_both_present(parsed_qchem_h2o_sp_data: CalculationResult):
    """
    ...
    assert parsed_qchem_h2o_sp_data.scf.energy == pytest.approx(parsed_qchem_h2o_sp_data.smd.g_enp_au, abs=ENERGY_TOL)


@pytest.mark.integration
def test_scf_energy_consistency(parsed_qchem_h2o_sp_data: CalculationResult):
    """
...
def test_scf_converged_status(parsed_qchem_h2o_sp_data: CalculationResult):
    """Regression test: verify SCF converged to expected status."""
    assert parsed_qchem_h2o_sp_data.scf is not None
    assert parsed_qchem_h2o_sp_data.scf.converged is EXPECTED_CONVERGED


@pytest.mark.regression
def test_scf_number_of_iterations(parsed_qchem_h2o_sp_data: CalculationResult):
    """Regression test: verify exact number of SCF iterations."""
    assert parsed_qchem_h2o_sp_data.scf is not None
    assert parsed_qchem_h2o_sp_data.scf.n_iterations == EXPECTED_N_ITERATIONS


@pytest.mark.regression
def test_scf_final_energy_value(parsed_qchem_h2o_sp_data: CalculationResult):
    """Regression test: verify exact final SCF energy value."""
    assert parsed_qchem_h2o_sp_data.scf is not None
    assert parsed_qchem_h2o_sp_data.scf.energy == pytest.approx(EXPECTED_SCF_ENERGY, abs=ENERGY_TOL)
...
@pytest.mark.regression
def test_all_seven_iterations_present(parsed_qchem_h2o_sp_data: CalculationResult):
    """Regression test: verify all 7 iterations are parsed, not truncated."""
    assert parsed_qchem_h2o_sp_data.scf is not None
    assert len(parsed_qchem_h2o_sp_data.scf.iterations) == 7

    # Verify iteration numbers are sequential
    for i, iteration in enumerate(parsed_qchem_h2o_sp_data.scf.iterations, start=1):
        assert iteration.iteration == i


@pytest.mark.regression
def test_smd_g_pcm_value(parsed_qchem_h2o_sp_data: CalculationResult):
    """Regression test: verify exact SMD polarization energy (G_PCM)."""
    assert parsed_qchem_h2o_sp_data.smd is not None
    assert parsed_qchem_h2o_sp_data.smd.g_pcm_kcal_mol == pytest.approx(EXPECTED_SMD_G_PCM_KCAL_MOL, abs=1e-4)


@pytest.mark.regression
def test_smd_g_cds_value(parsed_qchem_h2o_sp_data: CalculationResult):
    """Regression test: verify exact SMD non-electrostatic energy (G_CDS)."""
    assert parsed_qchem_h2o_sp_data.smd is not None
    assert parsed_qchem_h2o_sp_data.smd.g_cds_kcal_mol == pytest.approx(EXPECTED_SMD_G_CDS_KCAL_MOL, abs=1e-4)


@pytest.mark.regression
def test_smd_g_enp_value(parsed_qchem_h2o_sp_data: CalculationResult):
    """Regression test: verify exact SMD energy in solvent (G_ENP)."""
    assert parsed_qchem_h2o_sp_data.smd is not None
    assert parsed_qchem_h2o_sp_data.smd.g_enp_au == pytest.approx(EXPECTED_SMD_G_ENP_AU, abs=ENERGY_TOL)


@pytest.mark.regression
def test_smd_g_tot_value(parsed_qchem_h2o_sp_data: CalculationResult):
    """Regression test: verify exact SMD total free energy (G(tot))."""
    assert parsed_qchem_h2o_sp_data.smd is not None
    assert parsed_qchem_h2o_sp_data.smd.g_tot_au == pytest.approx(EXPECTED_SMD_G_TOT_AU, abs=ENERGY_TOL)


@pytest.mark.regression
def test_final_energy_equals_smd_total(parsed_qchem_h2o_sp_data: CalculationResult):
    """
    ...
    """
    assert parsed_qchem_h2o_sp_data.final_energy == pytest.approx(EXPECTED_TOTAL_ENERGY, abs=ENERGY_TOL)


@pytest.mark.regression
def test_last_iteration_convergence_marker(parsed_qchem_h2o_sp_data: CalculationResult):
    """
